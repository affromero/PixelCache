...
| `from difflogtest.utils.path import path_*` required          | Required. `difflogtest` is a runtime dep of pixelcache; the `path_*` helpers are reachable and used. The `Path \| str` type annotation may still be imported (`from pathlib import Path`) but operations (`Path.exists`, `Path.open`, etc.) must go through `path_*`. |
| `from difflogtest import get_logger` instead of `print()`     | Required.                                                                                                                                                                                                                                                             |
