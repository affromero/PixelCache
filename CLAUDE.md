...
| `from klogr.path import path_*` required          | Required. `klogr` is a runtime dep of pixelcache; the `path_*` helpers are reachable and used. The `Path \| str` type annotation may still be imported (`from pathlib import Path`) but operations (`Path.exists`, `Path.open`, etc.) must go through `path_*`. |
| `from klogr import get_logger` instead of `print()`     | Required.                                                                                                                                                                                                                                                             |
