...
def test_to_gray_l_mode(mid_rgb_np: UInt8[np.ndarray, "256 256 3"]) -> None:
    img = HashableImage(mid_rgb_np.copy())
    gray = img.to_gray()
    assert gray.dtype() == "L"
...
def test_to_binary(mid_rgb_np: UInt8[np.ndarray, "256 256 3"]) -> None:
    img = HashableImage(mid_rgb_np.copy())
...
def test_to_rgb_idempotent_on_rgb(
    mid_rgb_np: UInt8[np.ndarray, "256 256 3"],
) -> None:
    img = HashableImage(mid_rgb_np.copy())
    rgb = img.to_rgb()
...
def test_to_rgb_promotes_grayscale(
    mid_rgb_np: UInt8[np.ndarray, "256 256 3"],
) -> None:
...
def test_downsample(mid_rgb_np: UInt8[np.ndarray, "256 256 3"]) -> None:
    img = HashableImage(mid_rgb_np.copy())
